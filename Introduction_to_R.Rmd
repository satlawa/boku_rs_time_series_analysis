...
d;e
vec <- vector(mode="character",length=10) # create charcter vector of length 10
```
...
__Getting information about and dealing with objects in an Rsession:__
...
* You can explicitely coerce with `as.character()`, `as.double()`, `as.integer()`, `as.logical()` or `as.matrix()`.
...
Unknwon data sets can be explored as follows:
...
Normally commands are executed line by line. But there might be situtations when the sequence of statements and the number of times each is executed can vary. 
...
3. Use the funtion: `name(arglist)`
